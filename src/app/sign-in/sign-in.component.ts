...
export class SignInComponent implements OnInit {
  ...
  submiting() {
    ...
    const user =
      isNaN(this.profileForm.value.name) &&
      this.profileForm.value.name.length >= 3;

    console.log(user);
    if (
      isFinite(this.profileForm.value.name) ||
      this.profileForm.value.name.length < 3
    ) {
      this.vaild = 'your name must be alphabets and more than 3 letters ';
    } else if (
      !ptn.test(
        this.profileForm.value.email || this.profileForm.value.email.length < 8
      )
    ) {
      ...
      this.UsersService.register(this.profileForm.value).subscribe({
        ...
        error: (err) => {
          if (err.status == 400) {
            this.vaild =
              'you had created account please log in into your account ';
          } else {
